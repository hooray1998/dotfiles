...
function m() {
    ...
    case "$1" in
        ...
        "")            # m       - list all bookmarks
            local mark=`command ls -1 "$MARKPATH/" | MARKPATH="$MARKPATH" xargs -I'{}' \
                sh -c 'echo {},$(readlink "$MARKPATH/{}")' \
                | awk -F, '{printf("%-20s%-s\n", $1 , $2)}' \
                | fzf -n 1 --header="Jump to mark" --preview-window=:hidden \
                | cut -d' ' -f1`
            local dest="$(readlink "$MARKPATH/$mark" 2> /dev/null)"
